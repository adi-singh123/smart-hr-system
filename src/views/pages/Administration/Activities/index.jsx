...
const Activities = () => {
  ...
  const optionsSortValue = [
    { value: "sortTitle", label: "Sort by Title" },
  ...
  const handleLabelClick = () => setFocused(true);
  const handleInputBlur = () => inputValue === "" && setFocused(false);
  ...
  const handleDateChange2 = (date) => setSelectedDate2(date);

  const handleLabelClickTwo = () => setFocusedTwo(true);
  const handleInputBlurTwo = () => inputValueTwo === "" && setFocusedTwo(false);
  const handleInputChangeTwo = (e) => {
    const value = e.target.value;
    setInputValueTwo(value);
    if (value !== "" && !focusedTwo) setFocusedTwo(true);
  };
  ...
  const parseDate = (dateString) => new Date(dateString.replace(/(\d{2}) (\w{3,}) (\d{4})/, "$1 $2 $3"));
  ...
  const columns = [
      ...
      render: (text) => {
        ...
        return (
          <span style={{ color: isPast ? "red" : "inherit" }}>
            {isPast && <i className="las la-exclamation-triangle me-1" />}
            {text}
          </span>
        );
      },
    ...
    },
    {
      ...
      dataIndex: "Status",
      render: (text) => (
        <span className={`status-badge status-${text?.toLowerCase()}`}>{text}</span>
      ),
    },
    {
      title: "Action",
      render: () => (
        <div className="dropdown dropdown-action text-end">
          ...
          <div className="dropdown-menu dropdown-menu-right">
            <Link className="dropdown-item" to="#" data-bs-toggle="modal" data-bs-target="#edit_activity">
              <i className="fa fa-pencil m-r-5" /> Edit
            </Link>
            <Link className="dropdown-item" to="#" data-bs-toggle="modal" data-bs-target="#delete_activity">
