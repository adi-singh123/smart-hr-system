...
const EditSubCategory = () => {
  ...
  const { CategoryList, editSubCategoryObject } = useSelector((state) => state.category);
  ...
  // Ensure form updates when `editSubCategoryObject` changes
  useEffect(() => {
    if (editSubCategoryObject) {
      setValue("name", editSubCategoryObject.name || "");
      setValue("is_active", editSubCategoryObject.is_active?.toString() || "true");
      setValue("category_id", editSubCategoryObject.category_id || "");
    }
  }, [editSubCategoryObject, setValue]);
  ...
  const onSubmit = async (data) => {
    ...
    formData.append("id", editSubCategoryObject.id);
    ...
    try {
      const response = await dispatch(editSubCategory(formData));

      if (response?.payload?.status) {
        await dispatch(getSubCategories());

        const closeButton = document.querySelector("#edit_sub_category .btn-close");
  ...
  return (
    <div id="edit_sub_category" className="modal custom-modal fade" role="dialog">
              ...
              <div className="row">
                ...
                <div className="col-sm-6">
                  <div className="input-block mb-3">
                    ...
                    <select className="form-control" {...register("category_id", { required: "Category is required" })}>
