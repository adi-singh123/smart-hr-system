...
import { addChildSubCategory, getChildSubCategories, getSubCategories } from "../../../Redux/services/Category";
...
const AddChildSubCategory = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await dispatch(addChildSubCategory(formData));

      if (response?.payload?.status) {
        await dispatch(getChildSubCategories());

        const closeButton = document.querySelector("#add_child_sub_category .btn-close");
        if (closeButton) {
          closeButton.click();
          reset();
        }
  ...
  return (
    <div id="add_child_sub_category" className="modal custom-modal fade" role="dialog">
      <div className="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">Add Child Sub Category</h5>
